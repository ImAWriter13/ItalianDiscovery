...
open class DiscoveryTurbo : MainAPI() {
    override var lang: String = "it"
    override val hasMainPage: Boolean = true
    override var name: String = "Discovery Turbo"
    override var mainUrl: String = "https://discoveryturbo.it/"
    open var apiUrl: String = "https://public.aurora.enhanced.live"
    open var serviceIdentifier: String = "motortrend"
    open var category: String = "programmi"
    open var apiTokenRealm = "it"
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        val response =
            app.get("$apiUrl/site/page/homepage/?include=default&filter[environment]=$serviceIdentifier&v=2")
                .parsed<MediaResult>()

        val pages =
            response.blocks.filter { it.items.isNotEmpty() && it.type != "heroBlock" }
                .map { block ->
                    HomePageList(
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        val response =
            app.get("$apiUrl/site/search/page/?q=$query&include=default&filter[environment]=$serviceIdentifier&v=2&filter[type]=showpage&page[size]=20")
                .parsed<SearchRoot>()
    ...
    private fun MediaResult.toSearchResponse(): SearchResponse {
        return newMovieSearchResponse(
            name = title,
            url = "$mainUrl/$category/$slug",
            type = TvType.Movie
        ) {
            this.posterUrl = null //image?.url
    ...
    private fun EpisodeInfo.toSearchResponse(): SearchResponse {
        return newMovieSearchResponse(
            name = title.orEmpty(),
            url = fixUrl(link?.url ?: "/$category/${alternateId ?: url}"),
            type = TvType.Movie
        ) {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val response =
            app.get("$apiUrl/site/page/$slug/?include=default&filter[environment]=$serviceIdentifier&parent_slug=$category&v=2")
                .parsed<MediaResult>()
        ...
        if (seriesBlock != null) {
            ...
            return newTvSeriesLoadResponse(
                ...
                this.posterUrl = response.metaMedia.getOrNull(1)?.media?.url
                this.backgroundPosterUrl = response.metaMedia.firstOrNull()?.media?.url
